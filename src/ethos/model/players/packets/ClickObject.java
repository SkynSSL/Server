...
public class ClickObject implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		case FIRST_CLICK:
			...
			if (c.objectId == 29778 && Boundary.isIn(c, Boundary.RAIDS)) {
				c.getRaids().leaveGame(c);
				return;
			}
			if (c.objectId == 32653) {
				c.easyDifficulty = true;
				c.getRaids2().startRaid(); 
			}
			...
			switch (c.objectId) {
			case 2065:
				c.getPA().movePlayer(2729, 3489, 0);
				c.sendMessage("Welcome to the Seers rooftop agility course, climb up window-wall");
				break;
			case 31979:
			c.getShops().openShop(120);
			break; 
			case 32158: 
				c.sendMessage("With chisel in inventory use gem on jewelry station.");
				break;
			case 4006: 
				c.sendMessage("Mos Le'Harmless Cave");
				c.getPA().movePlayer(3748, 9374, 0);
				break;
			case 9354:
				if (c.getItems().playerHasItem(952, 1)) {
				if (System.currentTimeMillis() - c.foodDelay > 2500) {
					if (c.absX == 3501 && c.absY == 3233 || c.absX == 3509 && c.absY == 3233 || c.absX == 3501 && c.absY == 3222) {
						c.turnPlayerTo(c.absX, c.absY-1);
						c.startAnimation(830);
						if (Misc.random(10) >= 5) {
							c.getItems().addItem(22403, 1);
						} else {
							c.sendMessage("You found nothing.");
						}
					} 
				c.foodDelay = System.currentTimeMillis();
				} else {
				c.sendMessage("You can only dig this grave every 2 seconds.");
				}
				} else {
					c.sendMessage("I think I need something to dig with.");
				}
				break;
			case 30283:
