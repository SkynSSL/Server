...
public class NpcOptionTwo {
	...
	public static void handleOption(Player player, int npcType) {
		...
		case 2537:
		case 2536:
			player.getThieving().steal(Pickpocket.HAM_MEMBER, NPCHandler.npcs[player.rememberNpcIndex]);
			break;
		case 3100:
		case 3103:
		...
		case 3245:
			...
			break;
		case 3108:
			player.getThieving().steal(Pickpocket.ARDOUGNE_KNIGHT, NPCHandler.npcs[player.rememberNpcIndex]);
			break;
		case 3104:
		case 3105:
		...
		/*case 3257:
			...
			break; */
		/*case 5314:
				player.getPA().startTeleport(player.previousLastTeleportX, player.previousLastTeleportY, player.previousLastTeleportZ, "modern", false);
				player.sendMessage("Teleport to your destination then ::settele or (::st)(::pt)");
			break;*/
		case 3254:
		...
		case 3077:
			...
			player.getDH().sendNpcChat1("You've been playing "+Config.SERVER_NAME+" for " + time, 3077, "Hans");
		...
		case 324://SWORDIES+TUNA-CAGE+HARPOON
			if(player.getItems().playerHasItem(Fishing.isHarpoon[0])) {
				Fishing.attemptdata(player, 7);
			} else if(player.getItems().playerHasItem(Fishing.isHarpoon[1]) || player.getItems().isWearingItem(Fishing.isHarpoon[1])){
		...
		case 1045: // Santa
			...
			if (player.getItems().isWearingItems()) {
				player.sendMessage("You cannot bring your armor here, you'd freeze to the ground.");
				return;
			}
