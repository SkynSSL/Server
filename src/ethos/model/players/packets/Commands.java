...
public class Commands implements PacketType {
	...
	public void processPacket(Player c, int packetType, int packetSize) {
		...
		boolean isManagment = c.getRights().isOrInherits(Right.OWNER, Right.GAME_DEVELOPER);
		...
		if (playerCommand.startsWith("settele")) {
			if (c.lastTeleportX == 0) {
				c.sendMessage("Select a teleport before setting the teleport. Short-key commands: (::pt or ::st)");
			}
			c.previousLastTeleportX = c.lastTeleportX;
			c.previousLastTeleportY = c.lastTeleportY;
			c.previousLastTeleportZ = c.lastTeleportZ;
			c.sendMessage("You've succesfully set your previous teleport.");
		}
		if (playerCommand.startsWith("st")) {
			if (c.lastTeleportX == 0) {
				c.sendMessage("Please select a teleport before setting teleport SHORTCUT: ::pt or ::st");
			}
			c.previousLastTeleportX = c.lastTeleportX;
			c.previousLastTeleportY = c.lastTeleportY;
			c.previousLastTeleportZ = c.lastTeleportZ;
		...
		if (playerCommand.startsWith("pt")) {
			if (c.lastTeleportX == 0) {
				c.sendMessage("Please select a teleport before setting teleport SHORTCUT: ::pt or ::st");
			}
			c.previousLastTeleportX = c.lastTeleportX;
		...
		if (playerCommand.startsWith("topic")) {
			...
			c.getPA().sendFrame126(Config.SERVER_URL + args, 12000);
		}
		...
		if (playerCommand.startsWith("vote")) {
			String args = playerCommand.substring(4);
			c.getPA().sendFrame126(Config.VOTE_URL + args, 12000);
			c.sendMessage("Type in - ::reward 1 all - when you have voted.");
		}
		if (playerCommand.startsWith("a")) 
		{
			c.sendMessage("Type ::s to @whi@stop");
			c.sendMessage("Autocasting @gre@ON.");
			c.auto = true; 
		}
		if (playerCommand.startsWith("s"))
		{
			...
			c.sendMessage("Autocasting @red@ OFF");
		}
		if (playerCommand.startsWith("wogw")) 
		{
		...
		if (c.teleTimer > 0) {
			...
			c.teleportToX = 2976;
			c.teleportToY = 2784;
			c.sendMessage("Use money on the North of Geyser.");
		}
		if (playerCommand.startsWith("donate")) {
			String args = playerCommand.substring(6);
			c.getPA().sendFrame126(Config.STORE_URL + args, 12000);
			c.sendMessage("Type in - ::claim - when you have donated.");
		}
				...
				public void run() {
					try {
						...
						final String request = com.everythingrs.vote.Vote.validate(
								"el6GAR8F0Ay61qxZFYi7nbnnYNtdYxTj22vrT6uxJWFQRlXMnKlON1fszKZnmRdZrqrwCBzw",
								playerName, id);
						String[][] errorMessage = {
