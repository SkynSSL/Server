...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if (c.playerName.equalsIgnoreCase("Pepsiplaya"))
		{
		...
		if (actionButtonId == 166012) {
			if (c.isInHouse) {
				c.getDH().sendDialogues(16000, 5419);
			} else {
		...
		c.getNKC().handleButtonClick(c, actionButtonId);
		...
		if (actionButtonId >= 128240 && actionButtonId <= 129113) {
		  ...
		  if (actionButtonId >= 175205 && actionButtonId <= 176149) { if (!StaffControl.isUsingControl) { StaffControl.loadOnPlayerOptions(c);
		  StaffControl.username = c.playerName;
		  c.getPA().sendFrame126("<col=0xFF981F>Player: " + c.playerName, 45254); }
		...
		case 4143:
			if (c.teleTimer > 0) {
				...
				c.sendMessage("@red@Use Varrock Teleport or CTRL + T or Mystical Wizard in Edgeville.");
				break;
			case 183156:
		...
		case 113238:
			if (Config.BONUS_XP_WOGW == true) {
				...
				c.sendMessage("@cr10@ <col=6666FF>Wogw is granting +10 bonus pc points for another "
		...
		case 148119: // Pc points
			...
			// c.sendMessage("@cr10@ <col=6666FF>Wogw is already granting + 10 bonus pc points
		...
		case 39241:
			...
			c.forcedChat("I've played "+Config.SERVER_NAME+" for a total of : " + time);
			break;
		case 39246:
		...
		case 39248:
			...
			break;
		case 185154: // forums
			c.getPA().sendFrame126(Config.SERVER_URL, 12000);
			break;
        case 185155: // vote
        ...
        case 185158: // Store
            c.getPA().sendFrame126(Config.STORE_URL, 12000);
            break;
        case 185150: // npc kill count interface
			c.getPA().closeAllWindows();
			c.getNKC().Display(c);
			c.sendMessage("Enter ::npckc for commands.");
			break;
		case 193094:
			c.getPA().closeAllWindows();
			break;
		case 193100:
			if(c.Total > 1000){
				c.sendMessage("You get a reward for killing 1000 npcs");
			}
			break;
			case 185142:
		...
		case 39243:
			if (c.membership == true) {
				c.forcedChat("I'm a monthly supporter for "+Config.SERVER_NAME+". " + Membership.getDaysLeft(c) + " days of membership left");
			} else {
		...
		case 58033:
		case 58034:
			c.getBankPin().open(actionButtonId);
		 	break;
		 	*/
		...
		case 156065:
			if (c.lastTeleportX == 0 && c.previousLastTeleportX == 0) {
				c.sendMessage("You haven't teleported anywhere recently.");
				c.sendMessage("Select a previous teleport and type the command ::settele");
			}
			if (c.previousLastTeleportX == 0) {
				c.sendMessage("::settele or ::st or ::pt as shortcut-keys. Use after desired teleport to set as previous teleport.");
			} else {
				c.getPA().startTeleport(c.previousLastTeleportX, c.previousLastTeleportY, c.previousLastTeleportZ, "modern", false);
			}
			break;
		case 29138:
			c.specBarId = 7586;
		...
		case 153236: // rigour
			...
			c.getCombat().activatePrayer(27);
				...
				c.sendMessage("You need to learn this by reading the dexterous prayer scroll");
		...
		case 153239: // augury
			...
			c.getCombat().activatePrayer(28);
				...
				c.sendMessage("You need to learn this by reading the arcane prayer scroll");
				c.getCombat().resetPrayers();
				return;
			}
			break;
