...
import ethos.model.players.packets.objectoptions.impl.Raid2Objects;
...
public class ObjectOptionOne {
	...
	public static void handleOption(final Player c, int objectType, int obX, int obY) {
		if(objectType == 8210 && obX == 3010 && obY == 3378 || obX == 3083 && obY == 3500) {
			if(c.getRaked() == 0) {
				c.getTrees().RakePatch(c);
				return;
			}
			if(c.getRaked() == 1 && c.getCompost() == 0) {
				c.getTrees().addCompost(c);
				return;
			}
			if(c.getRaked() == 1 && c.getCompost() == 1 && c.getItems().playerHasItem(5312)) {
				c.getTrees().plantSeed(c, 5312, objectType, obX, obY);
				return;
			}
			if(c.getRaked() == 1 && c.getCompost() == 1 && c.getItems().playerHasItem(5313)) {
				c.getTrees().plantSeed(c, 5313, objectType, obX, obY);
				return;
			}
			if(c.getRaked() == 1 && c.getCompost() == 1 && c.getItems().playerHasItem(5315)) {
				c.getTrees().plantSeed(c, 5315, objectType, obX, obY);
				return;
			}
			if(c.getRaked() == 1 && c.getCompost() == 1 && c.getItems().playerHasItem(5316)) {
				c.getTrees().plantSeed(c, 5316, objectType, obX, obY);
				return;
			}
		} else {
		...
		//Raid2Objects.clickObject1(c, objectType, obX, obY);
		if (tree != null) {
			if(obX == 3083 && obY == 3500 || obX == 3010 && obY == 3378) {
				c.getTrees().cutDownTree(c, objectType, obX, obY);
			} else {
		...
		if (c.getRaids2().handleObjectClick(c,objectType)) {
			return;
		}
		...
		case 31990:
			...
			break;
		case 5791:
			c.sendMessage("@red@In memory of player mod Max 07/13/20.");
			break;
		case 8720:
		c.getShops().openShop(77);
		break;
		case 29777:
			c.getPA().movePlayer(1233, 3570, 0);
			break; 
		...
		case 1568:
			if (c.absX == 3044) {
				c.turnPlayerTo(obX, obY);
				c.getPA().walkTo(3045, 10342);
			} else if (c.absX == 3045) {
				c.turnPlayerTo(obX, obY);
				c.getPA().walkTo(3044, 10342);
			}
			break;
		...
		case 1569:
			if (c.absX == 3044) {
				c.turnPlayerTo(obX, obY);
				c.getPA().walkTo(3045, 10341);
			} else if (c.absX == 3045) {
				c.turnPlayerTo(obX, obY);
				c.getPA().walkTo(3044, 10341);
			}
			break;
		...
		case 4388:
			c.getPA().movePlayer(2991, 2780, 0);
			break;
		case 29150:
		...
		case 7184:
		case 26181:
			for(int itemId : Cooking.fishIds) { 
				c.turnPlayerTo(obX, obY);
				Cooking.cookThisFood(c, itemId, objectType);
			}
			break;
				case 1535://ham door
			if ((c.absX == 3158) && (c.absY == 9639)) {
				c.getPA().walkTo(0, +2);
			}
			else if ((c.absX == 3158) && (c.absY == 9641)) {
				c.getPA().walkTo(0, -2);
			}
			break;
		case 5492://ham trapdoor
			c.startAnimation(828);
			c.getPA().movePlayer(3149, 9652, 0);
			break;
		case 5493://ham ladder
			c.startAnimation(828);
			c.getPA().movePlayer(3166, 3251, 0);
			break;
		case 6150:
		case 2097:
		case 2783:
		...
		case 29347:// bossportal
			...
			break;
		case 20391:
			c.getPA().movePlayer(3283, 2807, 0);
			break;
		case 29345:// Training
		 ...
		 */
			case 26801:
				c.getShops().openShop(77);
				c.sendMessage("@red@ You currently have " + c.votePoints + " vote points.");
				c.sendMessage("@red@ Please type in ::vote to go to the site. And do ::reward to receive them!");
				break;
			case 31556:
		...
		case 21772:
			...
			if (cerb != null) {
				...
				c.getPA().movePlayer(1309, 1269, 0);
			}
			break;
		case 18621:
			if (System.currentTimeMillis() - c.cerbDelay > 5000) {
				Hydra hydra = c.createHydraInstance();
				if (c.getHydraLostItems().size() > 0) {
					c.getDH().sendDialogues(642, 5870);
					c.nextChat = -1;
					return;
				}
				...
				if (Server.getEventHandler().isRunning(c, "hydra")) {
					c.sendMessage("You're about to fight start the fight, please wait.");
					return;
				}
				c.getHydra().init();
			...
			break;
		case 18620:
			/* if (!Boundary.isIn(c, Boundary.HYDRA)) {
				return;
			} */
			//Hydra hydra = c.getHydra();

			/*if (hydra != null) {
				hydra.end(DisposeTypes.INCOMPLETE);
			} else {*/
				c.getPA().movePlayer(1358, 10260, 0);
				c.sendMessage("Pull the lever once more...");
			//}
			break;
		case 28900:
		...
		case 26760:
			...
			if (c.absX == 2972 && c.absY == 2717) {
				c.getDH().sendDialogues(750, -1);
			} else if (c.absX == 2972 && c.absY == 2716) {
				c.getPA().movePlayer(2972, 2717, 0);
			}
			if (c.absX == 3504 && c.absY == 3235) {
				c.getPA().movePlayer(3504, 3234, c.getIndex() * 4);
			} else if (c.absX == 3504 && c.absY == 3234) {
				c.getPA().movePlayer(3504, 3235, 0);
			}
			break;
		case 19206:
