...
public class FiveOptions {
	...
	public static void handleOption1(Player c) {
			...
			case 16052:
				c.getDH().sendDialogues(16053, 5419);
				break;
			
			case 16000:
				c.getDH().sendDialogues(16050, 5419);
				break;
			
			case 7600:
				c.getDH().sendDialogues(7601, 10);
				break;
			...
			case 7603:
				c.chosenCrawlingHand = true;
				c.getPA().showInterface(9000);
				break;
				
			case 7604:
				c.chosenSkeleton = true;
				c.getPA().showInterface(9000);
				break;
				
			case 7700:
				if (c.nlevel >= 5) {
				c.getDH().sendDialogues(7609, 10);
				} else {
					c.sendMessage("You need a necromancy level of 5 to do this.");
					c.getPA().closeAllWindows();
				}
				break;
			
			case 7501:
					c.getRaids().startRaid();
					c.easyDifficulty = true;
					c.getPA().closeAllWindows();
					c.sendMessage("@red@You Have Chosen: Easyses Difficulty");
				break;
			
			//StaffPotato
			case 861: //Referring To DialogueHandler Action "c.dialogueAction == ##"
			...
			case 858:
				c.getDH().sendDialogues(861, 2108);
				break;
			case 851:
	...
	public static void handleOption2(Player c) {
		...
		case 7600:
			if (c.nlevel >= 2) {
			c.getDH().sendDialogues(7603, 10);
			} else {
				c.sendMessage("You need a necromancy level of 2 to do this.");
				c.getPA().closeAllWindows();
			}
			break;
		
		case 7601:
			c.chosenChicken = true;
			c.getPA().showInterface(9000);
			break;
		...
		case 7603:
			c.chosenDesertLizard = true;
			c.getPA().showInterface(9000);
			break;
			
		case 7604:
			c.chosenChaosDruid = true;
			c.getPA().showInterface(9000);
			break;
			
		case 7700:
			if (c.nlevel >= 6) {
			c.getDH().sendDialogues(7611, 10);
			} else {
				c.sendMessage("You need a necromancy level of 6 to do this.");
				c.getPA().closeAllWindows();
			}
			break;
		...
		case 7501:
			c.getRaids().startRaid();
			c.normalDifficulty = true;
			c.getPA().closeAllWindows();
			c.sendMessage("@red@You Have Chosen: Normal Difficulty");
			break;
		...
		//StaffPotato
				...
				case 863:
					for (int i = 0; i < 22; i++) {
						c.playerLevel[i] = 1;
						c.playerXP[i] = c.getPA().getXPForLevel(1) + 0;
	...
	public static void handleOption3(Player c) {
		...
		case 7600:
			if (c.nlevel >= 3) {
			c.getDH().sendDialogues(7605, 10);
			} else {
				c.sendMessage("You need a necromancy level of 3 to do this.");
				c.getPA().closeAllWindows();
			}
			break;
		...
		case 7700:
			if (c.nlevel >= 7) {
			c.getDH().sendDialogues(7613, 10);
			} else {
				c.sendMessage("You need a necromancy level of 7 to do this.");
				c.getPA().closeAllWindows();
			}
			break;
		...
		case 7501:
			c.getRaids().startRaid();
			c.hardDifficulty = true;
			c.getPA().closeAllWindows();
			c.sendMessage("@red@You Have Chosen: Hard Difficulty");
			break;
	...
	public static void handleOption4(Player c) {
		...
		case 7600:
			...
			c.getDH().sendDialogues(7609, 10);
				...
				c.sendMessage("You need a necromancy level of 4 to do this.");
				c.getPA().closeAllWindows();
			}
			break;
		
		case 7601:
			c.chosenCaveBug = true;
			c.getPA().showInterface(9000);
			break;
		...
		case 7603:
			c.chosenKalphiteWorker = true;
			c.getPA().showInterface(9000);
			break;
			
		case 7604:
			c.chosenSandCrab = true;
			c.getPA().showInterface(9000);
			break;
			
		case 7700:
			if (c.nlevel >= 8) {
			c.getDH().sendDialogues(7615, 10);
			} else {
				c.sendMessage("You need a necromancy level of 8 to do this.");
				c.getPA().closeAllWindows();
			}
			break;
		
		case 7501:
			c.getDH().sendDialogues(7502, 10);
			break;
	...
	public static void handleOption5(Player c) {
		...
		case 16054:
			...
			c.getPA().closeAllWindows();
			c.sendMessage("Object Placed");
			break;
		
		case 7600: //Shows level 5-8
			c.getDH().sendDialogues(7700, 10);
			break;
		
		case 7601: //Level 1
			c.getDH().sendDialogues(7602, 10);
			break;
		...
		//StaffPotato
				case 861:
					c.getDH().sendOption5("@red@Fly", "@red@Reset Stats", "@red@Bank",
							"@red@Empty", "@cya@Next Page");
					c.dialogueAction = 863;
					break;
					
				case 858:
					c.getDH().sendOption5("Npcs", "Reload", "ItemSet",
							"Dialogue", "@cya@Next Page");
					break;
		case 711:
		case 77:
		case 801:
