...
public class TwoOptions {
	...
	public static void handleOption1(Player c) {
		...
		case 4007:
			...
			if (c.getItems().playerHasItem(2698, 1)) {
				...
				other.amDonated += 15;
				c.amDonated -= 15;
				other.sendMessage("$15 has been added to your total amount donated.");
				c.sendMessage("$15 has been removed from your amount donated and given to the other player.");
				c.updateRank();
				other.updateRank();
				c.getPA().closeAllWindows();
			}
		...
		case 4008:
			...
			if (c.getItems().playerHasItem(2699, 1)) {
				...
				other.amDonated += 35;
				c.amDonated -= 35;
				other.sendMessage("$35 has been added to your total amount donated.");
				c.sendMessage("$35 has been removed from your amount donated and given to the other player.");
				c.updateRank();
				other.updateRank();
				c.getPA().closeAllWindows();
			}
		...
		if (c.dialogueAction == 750) {
			int price = 500_000;
			if (c.absX == 2972 && c.absY == 2717) {
				if (c.getItems().playerHasItem(995, price)) {
					c.getPA().movePlayer(2972, 2716, 0);
					c.getItems().deleteItem2(995, price);
					c.getPA().removeAllWindows();
				} else {
					c.getDH().sendStatement("You need at least 500,000 gp to enter this area.");
				}
			}
			c.dialogueAction = -1;
			c.nextChat = -1;
		...
		if (c.dialogueAction == 125) {
			if (c.getItems().playerHasItem(8851, 200) || SkillcapePerks.ATTACK.isWearing(c)
				...
				c.getDH().sendNpcChat2("You need atleast 200 warrior guild tokens.",
						"You can get some by operating the armor animator.", 4289, "Kamfreena");
				c.nextChat = 0;
			}
			c.dialogueAction = -1;
			c.teleAction = -1;
		...
		if (c.dialogueAction == 4000) {
			...
			if (c.getItems().playerHasItem(2697, 1)) {
				...
				c.gfx100(263);
				c.amDonated += 5;
				c.sendMessage("$5 has been added to your total amount donated.");
				c.updateRank();
				c.getPA().closeAllWindows();
			}
		}
		if (c.dialogueAction == 4001) {
			...
			if (c.getItems().playerHasItem(2698, 1)) {
				...
				c.gfx100(263);
				c.amDonated += 15;
				c.sendMessage("$15 has been added to your total amount donated.");
				c.updateRank();
				c.getPA().closeAllWindows();
			}
		}
		if (c.dialogueAction == 4002) {
			...
			if (c.getItems().playerHasItem(2699, 1)) {
				...
				c.gfx100(263);
				c.amDonated += 35;
				c.sendMessage("$35 has been added to your total amount donated.");
				c.updateRank();
				c.getPA().closeAllWindows();
			}
		}
	...
	public static void handleOption2(Player c) {
		...
		case 1000:
			...
			c.getPA().itemOnInterface(386, 150, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(990, 3, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(995, 500000, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(2435, 15, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(3025, 10, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(2528, 1, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(7462, 1, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(2491, 1, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(2497, 1, 64503, spot1);
			spot1++;
			c.getPA().itemOnInterface(2503, 1, 64503, spot1);
			spot1++;
		...
		case 1001:
			int spot2 = 0;
			c.getPA().showInterface(64500);
			c.getPA().itemOnInterface(537, 25, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(1305, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(4587, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(1215, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(1377, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(4087, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(4585, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(11738, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(11286, 1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(-1, -1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(-1, -1, 64503, spot2);
			spot2++;
			c.getPA().itemOnInterface(-1, -1, 64503, spot2);
