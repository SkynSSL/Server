...
import ethos.model.npcs.bosses.demongod.DemonGod;
...
public class PlayerAssistant {
	...
	public void cameraMovement(Player c, int x, int y, int height, int speed, int angle) {
...
public void sendFrame107() {
	...
	public void castVengeance() {
		...
		c.gfx100(726);
		addSkillXP(112, Config.MAGIC, true);
		refreshSkill(6);
		c.vengOn = true;
		c.usingMagic = false;
	...
	public void alchemy(int itemId, String alch) {
		...
		case "low":
			if (System.currentTimeMillis() - c.alchDelay > 1000) {
				...
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[49][7], Config.MAGIC, true);
				refreshSkill(6);
			}
			break;
		...
		case "high":
			if (System.currentTimeMillis() - c.alchDelay > 2000) {
				...
				sendFrame106(6);
				addSkillXP(MagicData.MAGIC_SPELLS[50][7], Config.MAGIC, true);
				refreshSkill(6);
			}
			break;
		}
		...
		for (int[] aBoltData : boltData) {
			if (spell == aBoltData[0]) {
				if (bolt == aBoltData[1]) {
					...
					c.getPA().addSkillXP(aBoltData[2] * amount, Config.MAGIC, true);
	...
	public void giveLife() {
		...
		c.setVanguardDamageCounter1(0);
		c.setVanguardDamageCounter2(0);
		c.setVanguardDamageCounter3(0);
		c.setIceDemonDamageCounter(0);
		c.setSkeletalMysticDamageCounter1(0);
		c.setSkeletalMysticDamageCounter2(0);
		...
		if (!c.inDuelArena() && !Boundary.isIn(c, Boundary.DUEL_ARENA) && !Vorkath.inVorkath(c)
				...
				&& Highpkarena.getState(c) == null && !DemonGod.inGodDemon(c)
				&& !Boundary.isIn(c, Boundary.RAIDSTWO)) {
		...
		if (Boundary.isIn(c, PestControl.GAME_BOUNDARY)) {
			...
			c.getDH().sendNpcChat("It looks like Cerberus beat you.", "I'll give you back your items for 500,000GP.",
					"Talk to me when you're ready.");
		} else if (Boundary.isIn(c, Boundary.HYDRA)) {
			c.getPA().movePlayer(3102, 3485, 0);
			c.getCerberusLostItems().store();
			c.talkingNpc = 5870;
			c.getDH().sendNpcChat("Hydra has defeated you.", "I'll give you back your items for 500,000GP.",
					"Talk to me when you're ready.");
			...
			c.getPA().movePlayer(c.getRaids().getStartLocation().getX(),c.getRaids().getStartLocation().getY(),c.getRaids().getHeight(c.getRaids().raidLeader));
		} //else if (Boundary.isIn(c, Boundary.RAIDSTWO)) {
		//	c.getRaids2().handleDeath();
		//} 
		else if (Boundary.isIn(c, Boundary.OLM)) {
			c.getPA().movePlayer(c.getRaids().getStartLocation().getX(),c.getRaids().getStartLocation().getY(),c.getRaids().getHeight(c.getRaids().raidLeader));
	...
	public void levelUp(int skill) {
		...
		if (getLevelForXP(c.playerXP[skill]) == 99) {
			...
			PlayerHandler.executeGlobalMessage(
					"<img=10>[@blu@News@bla@] " + "@red@" + Misc.capitalize(c.playerName) + " @bla@has reached level 99 " + s.toString() + "!");
			DiscordMessager.sendPrestige("[News] " + Misc.capitalize(c.playerName) + " has reached level 99 " + s.toString() + "!");
		}
		c.dialogueAction = 0;
		c.nextChat = 0;
	}
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
        ...
        // each skill has a xp multiplier stored in the config file
        if (!c.getMode().getType().equals(ModeType.OSRS)) {
            amount *= Config.XpMultiplier(skill);
            amount *= c.prestige();
        }
		...
		if (dropExperience && amount > 0) {
			c.getPA().sendExperienceDrop(true, amount, skill);
		}

        // hitpoints are multipied inside the combat before getting here
        // melee rates
        if (skill == Config.ATTACK || skill == Config.DEFENCE || skill == Config.STRENGTH) {
            amount *= Config.MELEE_EXP_RATE;
        }
        // rage rates
        if (skill == Config.RANGED) {
            amount *= Config.RANGE_EXP_RATE;
        }

        // rage rates
        if (skill == Config.MAGIC) {
		...
		if (oldExperience < 200_000_000 && oldExperience + amount >= 200_000_000) {
			Skill s = Skill.forId(skill);
			PlayerHandler.executeGlobalMessage("@red@" + Misc.capitalize(c.playerName) + " has reached 200M XP in " + s.toString() + "!");
		}
	...
	public void addStarter() {
		...
		return;
			...
			DiscordMessager.sendNewPlayers("[@blu@New Player@bla@] " + Misc.capitalizeJustFirst(c.playerName) + " @bla@has logged in! Welcome!");
	...
	public void useOperate(int itemId) {
		...
		case 11283:
		case 11284:
			if (Boundary.isIn(c, Boundary.HYDRA) || Boundary.isIn(c, Zulrah.BOUNDARY) || Boundary.isIn(c, Boundary.CERBERUS_BOSSROOMS)
					|| Boundary.isIn(c, Boundary.SKOTIZO_BOSSROOM)) {
				return;
			}
		...
		case 9763:
			if (!Boundary.isIn(c, Boundary.EDGEVILLE_PERIMETER)) {
				c.sendMessage("This cape can only be operated within the home perimeter.");
				return;
			}
			if (c.inWild()) {
				return;
	...
	public void handleLoginText() {
		// modern
		c.getPA().sendFrame126("Mystic Teleports", 1300); // varrock
		c.getPA().sendFrame126("Use this to tele across the map", 1301); // varrock description
		c.getPA().sendFrame126("", 1325); // lumbridge
		c.getPA().sendFrame126("", 1326); // lumbridge description
		c.getPA().sendFrame126("", 1350); // falador
		c.getPA().sendFrame126("", 1351); // falador description
		c.getPA().sendFrame126("", 1382); // camelot
		c.getPA().sendFrame126("", 1383); // camelot description
		c.getPA().sendFrame126("", 1415); // ardougne
		c.getPA().sendFrame126("", 1416); // ardougne description
		c.getPA().sendFrame126("", 1454); // watchtower
		c.getPA().sendFrame126("", 1455); // watchtower description
		c.getPA().sendFrame126("", 7457); // trollheim
