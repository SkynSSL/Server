...
public class Crafting {
	...
	private int hideType = 0, makeId = 0, amount = 0, craftType = 0, exp = 0, index = 0;
		...
		for (int j = 0; j < gems.length; j++) {
			if (gems[j][0] == id) {
				if (c.playerLevel[Player.playerCrafting] >= gems[j][2]) {
					...
					c.getPA().addSkillXP((int) gems[j][3], Player.playerCrafting, true);
					if (gems[j][0] == 1631)
	...
	private void craftHides(int id) {
		for (int j = 0; j < amount; j++) {
			...
			c.getPA().addSkillXP((int) exp, Player.playerCrafting, true);
		}
		resetCrafting();
	}

	private int getItemToAdd() {
		if (craftType == 1) {
			return vambs[index];
		} else if (craftType == 2) {
			return chaps[index];
	...
	private int[] vambs = { 1065, 2487, 2489, 2491 };
	private int[] chaps = { 1099, 2493, 2495, 2497 };
	private int[] bodys = { 1135, 2499, 2501, 2503 };
	...
	private int[][] expsAndLevels = { { 1745, 62, 57 }, { 2505, 66, 70 }, { 2507, 73, 78 }, { 2509, 79, 86 } };

	private void openLeather(int item) {
		if (item == 1745) {
			...
			Server.getEventHandler().submit(new Event<Player>("skilling", c, 2) {
				...
				public void execute() {
					...
					if (attachment.getItems().playerHasItem(g.uncut)) {
						...
						attachment.getPA().addSkillXP((int) g.experience, Skill.CRAFTING.getId(), true);
	...
	enum CraftingGem {
		...
		CraftingGem(int uncut, int cut, int level, int experience) {
			this.uncut = uncut;
			this.cut = cut;
			this.level = level;
