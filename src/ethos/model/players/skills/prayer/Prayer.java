...
public class Prayer {
	...
	public void bury(Bone bone) {
		...
		player.getPA().addSkillXP((int) bone.getExperience() * (Boundary.isIn(player, Boundary.LAVA_DRAGON_ISLE) && bone.getItemId() == LAVA_DRAGON_BONES ? 4 : 1), Skill.PRAYER.getId(), true);
		player.getItems().deleteItem2(bone.getItemId(), 1);
		player.sendAudio(380);
	...
	public void alter(final int amount) {
		...
		player.getPA().addSkillXP((int) bone.getExperience() * 3, Skill.PRAYER.getId(), true);
		player.getItems().deleteItem2(bone.getItemId(), 1);
		...
		Server.getEventHandler().submit(new Event<Player>("skilling", player, 3) {
			...
			@Override
			public void execute() {
				if (player.disconnected || player.getSession() == null) {
					super.stop();
					return;
				}
				if (!player.getItems().playerHasItem(bone.getItemId())) {
