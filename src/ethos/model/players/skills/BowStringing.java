...
public class BowStringing extends StringingData {
	...
	public static void stringCrossBow(final Player c, final int itemUsed, final int usedWith) {
		if (c.playerSkilling[c.playerFletching]) {
			return;
		}
		...
		for (final stringingData g : stringingData.values()) {
			if (itemId == g.unStrung()) {
				...
				c.playerSkilling[c.playerFletching] = true;
				c.startAnimation(g.getAnimation());
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					@Override
					public void execute(CycleEventContainer container) {
						if (c.playerSkilling[c.playerFletching] == true) {
							if (c.getItems().playerHasItem(itemId)) {
								c.startAnimation(g.getAnimation());
							} else {
								container.stop();
					...
					@Override
					public void stop() {
						c.playerSkilling[c.playerFletching] = false;
					}
				}, 3);
				CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
					@Override
					public void execute(CycleEventContainer container) {
						if (c.playerSkilling[c.playerFletching] == true) {
							if (c.getItems().playerHasItem(itemId)) {
								...
								c.getPA().addSkillXP((int) g.getXP(), Config.FLETCHING, true);
