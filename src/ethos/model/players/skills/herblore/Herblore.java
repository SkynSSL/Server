...
public class Herblore {
	...
	public void clean(int itemId) {
		...
		herb.ifPresent(h -> {
			...
			player.getPA().addSkillXP((int) h.getExperience(), Skill.HERBLORE.getId(), true);
		...
		potion.ifPresent(p -> {
			...
			CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
				...
				public void execute(CycleEventContainer container) {
					...
					if (p.getResult().getId() == 22405) {
						player.nxp += 1;
					}
					player.getItems().addItem(p.getResult().getId(), p.getResult().getAmount());
					player.getPA().addSkillXP((int) p.getExperience(), Skill.HERBLORE.getId(), true);
