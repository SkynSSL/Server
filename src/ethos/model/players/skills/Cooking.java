...
public class Cooking extends SkillHandler {
	...
	private static void cookFish(Player c, int itemID, int xpRecieved, int levelRequired, int burntFish, int cookedFish, int object) {
		...
		c.playerSkillProp[Config.COOKING][0] = itemID;
		c.playerSkillProp[Config.COOKING][1] = xpRecieved;
		c.playerSkillProp[Config.COOKING][2] = levelRequired;
		c.playerSkillProp[Config.COOKING][3] = burntFish;
		c.playerSkillProp[Config.COOKING][4] = cookedFish;
		c.playerSkillProp[Config.COOKING][5] = object;
		c.playerSkillProp[Config.COOKING][6] = chance;
	...
	private static void resetCooking(Player c) {
	 ...
	 */
	private static boolean cookFish(Player c) {
		...
		return 0.45 + 0.70 * bonusChance >= random;
	}
		...
		Server.getEventHandler().submit(new Event<Player>("skilling", c, 2) {
			...
			public void execute() {
				...
				if (attachment.playerSkillProp[7][6] >= fishStopsBurning(attachment.playerSkillProp[7][0]) || cookFish(c)) {
					...
					attachment.getPA().addSkillXP(attachment.playerSkillProp[Config.COOKING][1], c.playerCooking, true);
					attachment.getItems().addItem(attachment.playerSkillProp[Config.COOKING][4], 1);
