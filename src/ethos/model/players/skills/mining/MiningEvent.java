...
public class MiningEvent extends Event<Player> {
	...
	public void execute() {
        ...
        if (attachment.currentPrestigeLevel == 2) {
    		if (Misc.random(15) == 0) {
    			attachment.getItems().addItem(mineral.getMineralReturn().generate(), 1);
    			attachment.sendMessage("@blu@You have mined an extra ore!");
    		}
        }
		...
		attachment.getPA().addSkillXP((int) regExperience, Skill.MINING.getId(), true);
		...
		if (Misc.random(mineral.getPetChance() / 100) == 10) {
		...
		if (Misc.random(mineral.getPetChance() / 75) == 10) {
		...
		if (Misc.random(mineral.getPetChance()) == 2 && attachment.getItems().getItemCount(13321, false) == 0
			...
			DiscordMessager.sendLootations("[News] " + attachment.playerName
					+ " mined a rock and formed the Rock golem pet!");
