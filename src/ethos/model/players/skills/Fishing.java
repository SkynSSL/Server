...
public class Fishing extends SkillHandler {
	...
	public static int[] isHarpoon = { 21028 };
	...
	public static int[][] data = { 
			...
			{ 15, 65, 3157, 3157, 3142, 105, 620, -1, -1, -1, 30200 }, // Karambwan
	...
	public static void attemptdata(final Player player, int npcId) {
		...
		for (int i = 0; i < data.length; i++) {
			if (npcId == data[i][0]) {
				...
				double experience = data[i][5] * (player.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.FISHING_EXPERIENCE * player.prestige()) * multiplier; // XP
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						...
						if (player.playerSkillProp[10][1] > 0) {
							...
							if (Misc.random(6000) == 5555) {
							 ...
							 if (Misc.random(player.playerSkillProp[10][10]) == 2 && player.getItems().getItemCount(13320, true) == 0 && player.summonId != 13320) {
								 ...
								 DiscordMessager.sendLootations("[News] " + player.playerName + " caught a fish and a Heron pet!");
						...
						player.prestigeNumber = 10;
						player.currentPrestigeLevel = player.prestigeLevel[player.prestigeNumber];
							
						  if (player.currentPrestigeLevel == 1) {
					    		if (Misc.random(30) == 0) {
					    			player.getItems().addItem(player.playerSkillProp[10][1], 1);
					    			player.sendMessage("@blu@You have caught an extra fish!");
					    		}
					        }
						...
						if (player.playerSkillProp[10][3] == 3157) {
