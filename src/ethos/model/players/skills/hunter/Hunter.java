...
public final class Hunter {
	...
	public static boolean claim(Player player, GlobalObject object) {
		...
		double percentOfXp = (trap.experience() * (player.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.HUNTER_EXPERIENCE * player.prestige()) / 100) * 2.5;

		Arrays.stream(trap.reward()).forEach(reward -> player.getItems().addItem(reward.getId(), reward.getAmount()));
		
		player.prestigeNumber = 21;
		player.currentPrestigeLevel = player.prestigeLevel[player.prestigeNumber];

		if (player.currentPrestigeLevel == 1) {
			if (Misc.random(10) == 0) {
				Arrays.stream(trap.reward()).forEach(reward -> player.getItems().addItem(reward.getId(), 1));
				player.sendMessage("@blu@You hunted an extra monster!");
			}
		}
		...
		if (player.currentPrestigeLevel == 3) {
			if (Misc.random(5) == 0) {
				Arrays.stream(trap.reward()).forEach(reward -> player.getItems().addItem(reward.getId(), 1));
				player.getItems().addItem(995, Misc.random(250));
				player.sendMessage("@blu@You hunted an extra monster!");
			}
		}
		...
		if (player.currentPrestigeLevel == 6) {
			if (Misc.random(5) == 0) {
				Arrays.stream(trap.reward()).forEach(reward -> player.getItems().addItem(reward.getId(), 1));
				player.getItems().addItem(995, Misc.random(1500));
				player.sendMessage("@blu@You hunted an extra monster!");
			}
		}
		...
		if (player.currentPrestigeLevel == 8) {
			if (Misc.random(5) == 0) {
				Arrays.stream(trap.reward()).forEach(reward -> player.getItems().addItem(reward.getId(), 1));
				player.getItems().addItem(995, Misc.random(2500));
				player.sendMessage("@blu@You hunted an extra monster!");
			}
		}
		...
		if (player.currentPrestigeLevel >= 10) {
			if (Misc.random(5) == 0) {
				Arrays.stream(trap.reward()).forEach(reward -> player.getItems().addItem(reward.getId(), 1));
				player.getItems().addItem(995, Misc.random(3500));
				player.sendMessage("@blu@You hunted an extra monster!");
			}
		}
        int SPOTTIER_CAPE = 10071;
		player.getPA().addSkillXP((int) (trap.experience() + (player.getItems().isWearingItem(SPOTTIER_CAPE) ? percentOfXp : 0)), 21, true);
		...
		case 10033:
			if (randomGray == 25 && player.getItems().getItemCount(13324, false) == 0 && player.summonId != 13324) {
				...
				DiscordMessager.sendLootations("[News] " + player.playerName + " caught a Gray Chinchompa pet!");
		...
		case 10008:
			if (randomRed == 15 && player.getItems().getItemCount(13323, false) == 0 && player.summonId != 13323) {
				...
				DiscordMessager.sendLootations("[News] " + player.playerName + " caught a Red Chinchompa pet!");
		...
		case 11959:
			if (randomBlack == 8 && player.getItems().getItemCount(13325, false) == 0 && player.summonId != 13325) {
				...
				DiscordMessager.sendLootations("[News] " + player.playerName + " caught a Black Chinchompa pet!");
