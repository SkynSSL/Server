		...
		task.ifPresent(task -> {
			...
			if (name.equals(task.getPrimaryName()) || ArrayUtils.contains(task.getNames(), name)) {
				...
				master.ifPresent(m -> {
					...
					case 6797:
						taskAmount--;
						player.getPA().addSkillXP((int) (player.getRechargeItems().hasAnyItem(13113, 13114, 13115) && Boundary.isIn(player, Boundary.SLAYER_TOWER_BOUNDARY) ? (int) (task.getExperience() * 1.10) : task.getExperience()), Skill.SLAYER.getId(), true);
						break;
					case 7663:
						if (npc.inWild()) {
							taskAmount--;
					...
					int bankCharge = Misc.random(100);
					if (taskAmount == 0) {
						...
						if (bankCharge >= 85) {
							player.bankCharges++;
							player.sendMessage("@red@You now have @dre@"+player.bankCharges+" @red@charges.");
						} else {
							return;
						}
						...
						case 6797:
							player.getPA().addSkillXP((int) BOSS_TASK_EXPERIENCE, Skill.SLAYER.getId(), true);
