...
public class Smelting {
	...
	private static void appendDelay(Player c, String usage) {
		...
		double percentOfXp = c.bar.getExp() * 2.5;
		c.prestigeNumber = 13;
		c.currentPrestigeLevel = c.prestigeLevel[c.prestigeNumber];
		  ...
		  if (c.currentPrestigeLevel == 1) {
	    		if (Misc.random(30) == 0) {
	    			...
	    			c.sendMessage("@blu@You have smelted an extra bar!");
	    		}
	        }
		...
		if (c.smeltAmount > 0) {
			...
			if (hasItems) {
				...
				if (c.bar.twoOres()) {
					...
					c.getItems().addItem(c.bar.getBar(), 1);
					c.getPA().addSkillXP((int) (c.bar.getExp() * (goldSmithGaunts ? percentOfXp : 1)), Config.SMITHING, true);
				} else {
					if (!Objects.equals(usage, "INFERNAL")) {
						c.startAnimation(899);
					}
					c.getItems().deleteItem(c.bar.getOre1(), 1);
					if ((c.bar.getBar() == 2351 && Misc.random(100) >= 25) && c.playerEquipment[c.playerRing] != 2568)
