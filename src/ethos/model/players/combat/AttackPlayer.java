...
public class AttackPlayer {
	...
	static void applyPlayerHit(final Player c, final int i, final Damage damage) {
		c.getCombat().applyPlayerMeleeDamage(i, 1, damage.getAmount(), damage.getHitmark());
	}

	private static boolean calculateBlockedHit(Player c, int defence) {
		if (defence > 500 && Misc.random(5) == 1)
			return true;
		if (defence > 450 && Misc.random(5) == 1)
			return true;
		if (defence > 400 && Misc.random(6) == 1)
			return true;
	...
	static void applyPlayerMeleeDamage(Player c, int i, int damageMask, int damage, Hitmark hitmark) {
	...
	public static void addCombatXP(Player c, CombatType type, int damage) {
		...
		case MELEE:
			...
			c.getPA().addSkillXP((int) Math.ceil(damage * 4 * Config.MELEE_EXP_RATE) / 3, Config.HITPOINTS, true);

			switch (c.fightMode) {
			default:
			case 0: // Accurate
				c.getPA().addSkillXP((int) Math.ceil(damage * 4), Config.ATTACK, true);
				break;
			case 1: // Block
				c.getPA().addSkillXP((int) Math.ceil(damage * 4), Config.DEFENCE, true);
				break;
			case 2: // Aggressive
				c.getPA().addSkillXP((int) Math.ceil(damage * 4), Config.STRENGTH, true);
				break;
			case 3: // Controlled
				c.getPA().addSkillXP((int) Math.ceil(damage * 4) / 3, Config.ATTACK, true);
				c.getPA().addSkillXP((int) Math.ceil(damage * 4) / 3, Config.DEFENCE, true);
				c.getPA().addSkillXP((int) Math.ceil(damage * 4) / 3, Config.STRENGTH, true);
				break;
			}
			break;

		case RANGE:
			c.getPA().addSkillXP((int) Math.ceil(damage * 4 * Config.RANGE_EXP_RATE) / 3, Config.HITPOINTS, true);
			switch (c.fightMode) {
			case 0: // Accurate
			case 2: // Rapid
                c.getPA().addSkillXP((int) Math.ceil(damage * 4), Config.RANGED, true);
				break;
			case 1: // Block
			case 3:
                c.getPA().addSkillXP((int) Math.ceil(damage * 2), Config.RANGED, true);
                c.getPA().addSkillXP((int) Math.ceil(damage * 2), Config.DEFENCE, true);
				break;
			}
			break;
		case MAGE:
			c.getPA().addSkillXP((int) Math.ceil(damage * 4 * Config.MAGIC_EXP_RATE) / 3, Config.HITPOINTS, true);
            c.getPA().addSkillXP((int) Math.ceil(damage * 4), Config.MAGIC, true);
			break;
		}
	}

	static void playerDelayedHit(final Player c, final int i, final Damage d) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (attacker.magicDef) {
					attacker.getPA().addSkillXP((int) (damage * 2), Config.DEFENCE, true);
	...
	public static int[] simulateCombatDamage(Player attacker, Player defender, CombatType combatType, Special special) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				if (attacker.getMode().isPVPCombatExperienceGained()) {
					addCombatXP(attacker, CombatType.MAGE, damage + (Math.max(damage2, 0)));
				}
				attacker.getPA().refreshSkill(6);
			}
		}
	...
	private static void resetSpells(Player c) {
	...
	static void attackPlayer(Player c, int i) {
		if (PlayerHandler.players[i] == null) {
			return;
		}
