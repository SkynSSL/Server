...
public class RangeMaxHit extends RangeData {
	...
	public static int calculateRangeAttack(Player c) {
		...
		if (c.npcIndex > 0 && c.getItems().isWearingItem(22550)) {
			rangeLevel += 2.1;
		}
		if (c.playerIndex > 0 && c.getItems().isWearingItem(22550)) {
			rangeLevel += 2.1;
		}
		if (c.playerIndex > 0 && c.getItems().isWearingItem(20997)) {
			rangeLevel += 0.8;
		}
		if (c.playerIndex > 0 && c.getItems().isWearingItem(12926)) {
			rangeLevel += 1.0;
		}
		if (c.fullVoidRange()) {
		...
		if (c.npcIndex > 0 && c.getSlayer().getTask().isPresent()) {
			...
			if (c.getSlayer().getTask().isPresent()) {
				if (npc != null && c.getSlayer().getTask().get().matches(npc.getDefinition().getNpcName()) || npc.npcType == 7413) {
					if (!c.getItems().isWearingItem(11865) || c.getItems().isWearingItem(19641) || c.getItems().isWearingItem(19645) || c.getItems().isWearingItem(19649) || c.getItems().isWearingItem(21266) || c.getItems().isWearingItem(21890)) {
						rangeLevel *= 1.35;
					}
				}
			}
			if (c.getSlayer().getTask().isPresent()) {
				if (npc != null && c.getSlayer().getTask().get().matches(npc.getDefinition().getNpcName()) || npc.npcType == 7413) {
					if (!c.getItems().isWearingItem(11864) || c.getItems().isWearingItem(19639) || c.getItems().isWearingItem(19643) || c.getItems().isWearingItem(19647) || c.getItems().isWearingItem(21264) || c.getItems().isWearingItem(21888)) {
						rangeLevel *= 1.15;
					}
				}
			}
			if (c.getItems().isWearingItem(21012, c.playerWeapon)) {
				if (Misc.linearSearch(Config.DRAGON_IDS, npc.npcType) != -1) {
					rangeLevel *= 1.70;
				}
			}
	...
	public static boolean wearingCrawsBow(Player c) {
		return c.playerEquipment[3] != -1 && Item.getItemName(c.playerEquipment[3]).toLowerCase().contains("craw's bow");
	}
