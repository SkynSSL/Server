	...
	public static void calculateCombatDamage(Player attacker, NPC defender, CombatType combatType, Special special) {
		...
		if (Objects.nonNull(attacker) && Objects.nonNull(defender)) {
			if (combatType.equals(CombatType.MELEE)) {
				...
				hitmark2 = damage3 > 0 ? Hitmark.HIT : Hitmark.MISS;
				...
				if (defender.npcType != 7413) {
					AttackPlayer.addCombatXP(attacker, CombatType.RANGE, damage + (damage2 > 0 ? damage2 : 0) + (damage3 > 0 ? damage3 : 0));
				}
				...
				if (attacker.magicDef) {
					if (defender.npcType != 7413) {
						attacker.getPA().addSkillXP((int) (damage * 3), Config.DEFENCE, true);
		...
		if (damage3 > -1) {
			attacker.getDamageQueue().add(new Damage(defender, damage3, delay+1, attacker.playerEquipment, hitmark2, combatType));
		}
	}
...
//				break;
			...
			case 7604: //Skeletal mystic
				c.setSkeletalMysticDamageCounter1(c.getSkeletalMysticDamageCounter1() + damage.getAmount());
			break;
			case 7605: //Skeletal mystic
				c.setSkeletalMysticDamageCounter2(c.getSkeletalMysticDamageCounter2() + damage.getAmount());
				break;
			case 7606: //Skeletal mystic
				c.setSkeletalMysticDamageCounter3(c.getSkeletalMysticDamageCounter3() + damage.getAmount());
				break;
			...
			case 7527:
				c.setVanguardDamageCounter1(c.getVanguardDamageCounter1() + damage.getAmount());
				break;
				
			case 7528:
				c.setVanguardDamageCounter2(c.getVanguardDamageCounter2() + damage.getAmount());
				break;
				
			case 7529: //vanguard
				c.setVanguardDamageCounter3(c.getVanguardDamageCounter3() + damage.getAmount());
				break;
				
			case 7563: //Muttadile
				c.setMuttadileDamageCounter(c.getMuttadileDamageCounter() + damage.getAmount());
				break;
				
			case 7566: //Vasa Nistirio
				c.setVasaDamageCounter(c.getVasaDamageCounter() + damage.getAmount());
				break;
			...
			case 8615:
				if (Boundary.isIn(c, Boundary.HYDRA)) {
					c.getHydra().hydraSpecials();
				} else {
					damage.setAmount(0);
					c.sendMessage("@red@You should keep yourself in the middle so you don't get hit by dragonfire.");
				}
				break;
			case 5862:
	...
	public static void attackNpc(Player c, int i) {
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if (c.attackTimer <= 0) {
				...
				if (c.usingBallista)
					distance = 6;
				if (c.usingBow || c.usingMagic || c.autocasting || c.playerEquipment[c.playerWeapon] == 11907 || c.playerEquipment[c.playerWeapon] == 12899 || c.playerEquipment[c.playerWeapon] == 22550)
					distance = 8;
				...
				if (!c.getCombat().correctBowAndArrows() && Config.CORRECT_ARROWS && c.usingBow && !c.getCombat().usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 9185
						&& c.playerEquipment[c.playerWeapon] != 4734 && c.playerEquipment[c.playerWeapon] != 11785 && c.playerEquipment[c.playerWeapon] != 21012 && c.playerEquipment[c.playerWeapon] != 12926 && c.playerEquipment[c.playerWeapon] != 19478 && c.playerEquipment[c.playerWeapon] != 19481 && c.playerEquipment[c.playerWeapon] != 22550) {
				...
				if (c.usingBow && Config.CRYSTAL_BOW_DEGRADES) { // crystal bow
					...
					if (c.crystalBowArrowCount >= 250 && c.playerEquipment[c.playerWeapon] != 22550 && c.playerEquipment[c.playerWeapon] != 20997) {
						switch (c.playerEquipment[c.playerWeapon]) {
