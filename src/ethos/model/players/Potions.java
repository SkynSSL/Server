...
public class Potions {
	...
	/**
	 * <h1>Crushes the empty vial after drinking a potion!</h1> The
	 * crushVialIfDonator method implements an QoL that simply crushes an empty vial
	 * after the player drinks a potion.
	 * <p>
	 * 
	 * @param c    This Player to be affected.
	 * @param slot This slot where the potion is at.
	 * 
	 */

	public void crushVialIfDonator(Player c, int slot) {
		if (c.getRights().isOrInherits(Right.CONTRIBUTOR) || c.getRights().isOrInherits(Right.HELPER)) {
			c.getItems().deleteItem(229, slot, 1);
		}
	}

	public void drinkAntifire(int replaceItem, int slot, long duration) {
        drinkPotion(replaceItem, slot);
	...
	public void drinkPotion(int replaceItem, int slot) {
		c.startAnimation(829);
		c.playerItems[slot] = replaceItem + 1;
        if (replaceItem == 229) { // empty vial
            crushVialIfDonator(c, slot);
        }
