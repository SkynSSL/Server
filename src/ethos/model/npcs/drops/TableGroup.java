						...
						items.add(item);
							if (policy.equals(TablePolicy.VERY_RARE) || policy.equals(TablePolicy.RARE)) {
								if (Item.getItemName(item.getId()).toLowerCase().contains("cowhide")
										...
										|| Item.getItemName(item.getId()).toLowerCase().contains("steel")
										...
										|| Item.getItemName(item.getId()).toLowerCase().contains("grimy")
										|| Item.getItemName(item.getId()).toLowerCase().contains("seeds")
										...
										|| Item.getItemName(item.getId()).toLowerCase().contains("looting")
										|| Item.getItemName(item.getId()).toLowerCase().contains("magic log")
										...
										|| Item.getItemName(item.getId()).toLowerCase().contains("key")
										|| Item.getItemName(item.getId()).toLowerCase().contains("helm of neitiznot")
										|| item.getId() >= 554 && item.getId() <= 566) {
									
								} else {
									PlayerHandler.executeGlobalMessage(
											"[@red@Lootations@bla@]@blu@ @cr9@ <col=255><img="+rights+">" + Misc.capitalize(player.playerName) + "</col> received @blu@" + ItemAssistant.getItemName(item.getId()) + "@bla@.");
									player.sendAudio(436);
									DiscordMessager.sendLootations("[Lootations] " + Misc.capitalize(player.playerName) + " received " + ItemAssistant.getItemName(item.getId()) + ".");
								}
							}
						}
					}
