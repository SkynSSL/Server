...
public class ShopAssistant {
	...
	public void sellToShopPrice(int removeId, int removeSlot) {
		...
		if (IsIn == false) {
			...
			if (c.myShopId == 12) {
				...
				c.sendMessage(ItemAssistant.getItemName(removeId) + ": shop will buy for " + ShopValue + " raid points" + ShopAdd);
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (c.myShopId != 115) {
			if (itemID == 863 || itemID == 11230 || itemID == 869 || itemID == 868 || itemID == 867 || itemID == 866 || itemID == 4740 || itemID == 9244 || itemID == 11212
					...
					|| itemID == 886 || itemID == 888 || itemID == 890 || itemID == 11179 || itemID == 995) {
	...
	public void handleOtherShop(int itemID, int amount) {
		...
		if (c.myShopId == 118) {
			int itemValue = getSpecialItemValue(itemID) * amount;
			if (!c.getItems().playerHasItem(11179, itemValue)) {
				c.sendMessage("You do not have enough raid points to purchase this.");
				return;
			}
			c.getItems().deleteItem(11179, itemValue);
			c.getItems().addItem(itemID, amount);
			c.getItems().resetItems(3823);
			return;
		}
