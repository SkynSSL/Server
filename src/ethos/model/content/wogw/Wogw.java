...
public class Wogw {
	...
	public static void donate(Player player, int amount) {
		 ...
		 */
		String towards =Objects.equals(player.wogwOption, "pc") ? "+ 10 bonus PC Points!" : Objects.equals(player.wogwOption, "experience") ? "double experience!" : Objects.equals(player.wogwOption, "drops") ? "double drops!" : "";
		...
		case "pc":
			if (MONEY_TOWARDS_PC_POINTS + amount >= 50000000 && PC_POINTS_TIMER == 0) {
				...
				PlayerHandler.executeGlobalMessage("@cr10@[@blu@WOGW@bla@] <col=6666FF>It is now granting everyone 1 hour of + 10 bonus pc points.");
				DiscordMessager.sendAnnouncement("[WOGW] Is now giving everyone + 10 PC points.");
		...
		case "drops":
			if (MONEY_TOWARDS_DOUBLE_DROPS + amount >= 100000000 && DOUBLE_DROPS_TIMER == 0) {
				...
				DiscordMessager.sendAnnouncement("[WOGW] Is now granting everyone 1 hour of double drop rate.");
	...
	public static void donateItems(Player player, int amount) {
		 ...
		 * Updating latest donators
		 */
		//String towards = player.wogwOption == "pc" ? "+ 10 bonus PC Points!" : player.wogwOption == "experience" ? "double experience!" : player.wogwOption == "drops" ? "double drops!" : "";
		//player.sendMessage("You successfully donated " + Misc.format((int) player.wogwDonationAmount) + "gp to the well of goodwill towards");
		//player.sendMessage(towards);
		//Wogw.lastDonators[Wogw.slot] = "" + Misc.formatPlayerName(player.playerName) + " donated " + Misc.getValueWithoutRepresentation(player.wogwDonationAmount) + " towards " + towards;
		player.getPA().sendFrame126(Wogw.lastDonators[Wogw.slot], 38033 + Wogw.slot);
	...
	private static void handleMoneyToExperience(int amount) {
		...
		if (MONEY_TOWARDS_EXPERIENCE + amount >= 50000000 && EXPERIENCE_TIMER == 0) {
			...
			DiscordMessager.sendAnnouncement("[WOGW] It is now granting everyone 1 hour of double experience.");
	...
	public static void appendBonus() {
			if (MONEY_TOWARDS_EXPERIENCE >= 50000000) {
				...
				DiscordMessager.sendAnnouncement("[WOGW] It is now granting everyone 1 more hour of double experience.");
				EXPERIENCE_TIMER += TimeUnit.HOURS.toMillis(1) / 600;
			...
			if (MONEY_TOWARDS_PC_POINTS >= 50000000) {
				PlayerHandler.executeGlobalMessage("@cr10@[@blu@WOGW@bla@] <col=6666FF>The Well of Goodwill was filled above the top!");
				PlayerHandler.executeGlobalMessage("@cr10@[@blu@WOGW@bla@] <col=6666FF>It is now granting everyone 1 more hour of + 10 bonus pc points.");
				DiscordMessager.sendAnnouncement("[WOGW] Is now granting everyone 1 more hour of + 10 bonus pc points.");
				PC_POINTS_TIMER += TimeUnit.HOURS.toMillis(1) / 600;
			...
			if (MONEY_TOWARDS_DOUBLE_DROPS >= 100000000) {
				...
				DiscordMessager.sendAnnouncement("[WOGW] It is now granting everyone 1 more hour of double drop rate");
				DOUBLE_DROPS_TIMER += TimeUnit.HOURS.toMillis(1) / 600;
