...
public class TeleportHandler {
	...
	String[] monsterNames = { "@yel@Select Your Monster Teleport", "@or1@Cow-Chickens @gre@(Level-2)", "@or1@Rock Crabs @gre@(Level-13)", "@or1@Girl-Chaos-Druids @gre@(Level-13) @red@(Wilderness)", "@or1@Yaks @gre@(Level-22)",
			...
			"@or1@Demonic Gorillas @red@(Level-275)", "@or1@Mithril Dragons @red@(Level-304)", "@or1@Slayer Tower @mag@(Variety)", "@or1@Taverly Dungeon @mag@(Variety)", "@or1@Stronghold Cave @mag@(Variety)", "@or1@Hydra @red@(Level-426)", "", "", "", "" };
	String[] minigameNames = { "@yel@Select Your Minigame Teleport", "@gre@Raids @or1@(Great Rewards)", "@gre@Warriors Guild @or1@(Defenders)", "@gre@Pest Control @or1@(Void)",
			"@gre@Fight Caves @or1@(Firecape)", "@gre@Barrows @or1@(Barrows Armor)", "@gre@Clan Wars @red@(Under Construction)", "@gre@Shayzien Assault @or1@(Fighter Torso)", "@gre@Mage Arena @red@(Under Construction)", "@gre@Duel Arena @or1@(Gamble/1v1)", "@gre@Raids 2@or1@(Great Rewards)", "", "", "", "",
			"", "", "", "", "", "" };
	...
	public void handleTeleports(Player player, int Id) {
		...
		case 180227: //vorkath
			if (player.teleSelected == 0) { // Monsters - Stronghold Cave
				player.getPA().startTeleport(1352, 10254, 0, "modern", false);
			}
		...
		case 180231: //vorkath
			if (player.teleSelected == 0) { // Monsters - Stronghold Cave
				player.getPA().startTeleport(3748, 9374, 0, "modern", false);
			}
			player.getPA().startTeleport(2272, 4050, 0, "modern", false);
			//player.sendMessage("This has been temporarily disabled");
			break;
		...
		case 180209:
			...
			if (player.teleSelected == 0) { // Monsters - Smoke Devils
				...
				player.getPA().startTeleport(3651, 3214, 0, "modern", false);
