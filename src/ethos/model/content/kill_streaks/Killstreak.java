...
public class Killstreak {
	...
	private void reward(Type type) {
		...
		Optional<KillstreakReward> reward = Arrays.stream(type.rewards).filter(s -> s.killstreak == streak).findFirst();
		reward.ifPresent(killstreakReward -> killstreakReward.append(player));
	...
	public enum Type {
		...
		public static Type get(String name) {
			Optional<Type> op = Arrays.stream(values()).filter(t -> t.name().equals(name)).findFirst();
