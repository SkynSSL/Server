...
public class LootingBag {
     ...
     * @return
     */
    private boolean configurationPermitted() {
    ...
    public void depositAllLootBag() {
        ...
        //    player.sendMessage("You must be in edge bank to do this.");
        //    return;
        //}
    ...
    public boolean withdraw(int id, int amount) {
        ...
        if (!Boundary.isIn(player, Boundary.HOME) && !player.inBank() || player.inWild()) {
            player.sendMessage("Please go home to withraw items from your looting bag.");
            return false;
        }
    ...
    private boolean addItemToList(int id, int amount) {
        for (LootingBagItem item : items) {
            if (item.getId() == id) {
    ...
    private List<GameItem> loot = new ArrayList<GameItem>();
    ...
    private void sendItems() {
        ...
        for (int i = 0; i < 28; i++) {
			...
			PlayerAssistant.sendItems(player, 39349, loot, 28);
        }
    }
