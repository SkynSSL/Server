...
public class PrestigeSkills {
	...
	private final int MAX_PRESTIGE = 10;
	...
	public void prestige() {
		...
		if (player.canPrestige) { // If the skill is 99
			...
			PlayerHandler.executeGlobalMessage("@bla@[@red@News@bla@] @cr19@ @red@"+ Misc.capitalize(player.playerName) + " @bla@prestiged their @red@" + Config.SKILL_NAME[player.prestigeNumber] + " @bla@[@red@" + player.currentPrestigeLevel + "@bla@]");
			DiscordMessager.sendPrestige("[News] "+ Misc.capitalize(player.playerName) + " prestiged their " + Config.SKILL_NAME[player.prestigeNumber] + " [" + player.currentPrestigeLevel + "]");
			player.relicUpgradePrestige();
		} else {
