...
public class Listing {
    ...
    resetEverything(c);
  	...
  	public static void loadRecentPageTwo(Player c) {
		c.pageId = 0;
		c.searchId = 3;
		c.getPA().sendFrame126("Trading Post - Recent listings", 48019);
		c.getPA().showInterface(48000);
		...
		int total = 0;
    resetEverything(c);
		for(int i = NEXT_SALE_ID - 7; i > 0; i--) {
			Sale sale = getSale(i);
			if(sale.hasSold()) continue;
			total++;
			sales.add(sale);
	...
	public static void buyListing(Player c, int slot, int amount) {
		...
		if(sales.hasSold() || sales.getTotalSold() == sales.getQuantity()) {
			c.sendMessage("Item removed.");
			return;
		}
